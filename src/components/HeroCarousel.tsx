      ...
      <div aria-hidden className="absolute inset-0 z-0">
        {SLIDES.map((slide, i) => (
          ...
          >
            ...
            {/* cinematic vignette */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(0,0,0,0)_30%,rgba(10,4,4,0.7)_100%)]" />
            <div className="absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t from-crimson via-crimson/70 to-transparent" />
            <div className="absolute inset-y-0 left-0 w-full bg-gradient-to-r from-crimson/85 via-crimson/40 to-transparent md:w-1/2" />
          </div>
        ))}
      </div>

      {/* Foreground content slot — flows in normal layout, no overflow */}
      {children && (
        <div className="relative z-10 w-full pb-16">{children}</div>
      )}
